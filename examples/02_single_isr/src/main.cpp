...
// ISR do hw_timer no ESP32: só executa o maestro de passos.
...
// Callback esp_timer em alvos não-ESP32: mesmo papel que a ISR (tick periódico).
...
// Modo ISR: regista motor, inicia timer periódico e dispara movimento.
void setup() {
  Serial.begin(115200);
  delay(200);
...
// Só planner no loop; passos vêm do timer; LED e mensagem de fim como no exemplo cooperativo.
void loop() {
  drivers.run();
